...
6.  **Popup materialization cost:** default compiled popup switching now removes the bounded CLI-launch overhead; its final same-daemon A/B saved `4.168 ms` at process completion and `5.354 ms` at renderer-observed open. Initial latency is still dominated by SketchyBar/WindowServer layout and scales with visible row count, so keep root menus shallow rather than adding more hot-path transport layers.
