...
- The focused path now commits one versioned `space + display` context only after a successful SketchyBar apply. Same-display moves make the old space idle, while cross-display moves keep the old display's space visible-inactive; both style changes travel in one request.
- Missing, legacy, malformed, failed-apply, or failed-publication selection context fails closed to a full visual pass; malformed includes extra lines and unterminated trailing bytes. Reload removes the prior selection generation. A concurrent `front_app_switched` waits outside the visual lock for topology, applies its latest event after release, and fails open after a two-second stale-lock bound instead of being dropped. If an ordinary visual pass already owns the lock, the native `file_lock` helper keeps one waiter queued for up to six seconds, re-reads current focus after release, and opens the retry slot before sampling so a post-snapshot event can queue behind it while additional contenders coalesce.
...
- The delayed startup visual sync now runs as `startup_sync`, uses its own wider cooldown window, and waits outside the visual lock for an active topology refresh to finish. Normal startup avoids a duplicate full visual pass; if the lock remains after the two-second bound, the fallback ignores any stale marker and proceeds. Reload discards the previous runtime's marker, and new authoritative markers publish only after a successful SketchyBar apply, so failed discovery/apply cannot poison recovery.
- `space_visuals.sh` now resolves visible-space apps with scoped window data instead of a single global window snapshot. When `bin/space_visual_helper` is available, one helper invocation batches those visible-space app lookups and avoids per-space shell/jq parsing; the helper uses exact-argument `posix_spawn`, drains output while each query runs, and bounds timeout cleanup to the complete child process group. Missing app glyphs then resolve through one `app_icon.sh --batch` call before the SketchyBar apply.
- Full visual passes preload the persisted space-icon cache once in the parent shell before entering the space loop, so command substitutions reuse populated arrays instead of rebuilding discarded cache state.
...
- `plugins/lib/space_style.sh` now defines focused, visible-inactive, idle, and hover styles. `space_visuals.sh` writes the full per-space style state under `cache/space_visuals/style_state/`, and `space.sh` restores from that state after hover instead of guessing from the retired space-only selection marker.
- `space_visuals.sh` caches focused/visible/idle style argument arrays once per run, keeps normal style paths and disabled phase probes in the parent shell, batches helper-backed visible app/glyph lookup when available, skips rewriting unchanged style-state files, and can emit detailed phase fields when run with `BARISTA_SPACE_VISUAL_PHASE_METRICS=1`; those fields appear under `Space visual refreshes` in `barista-stats.sh show`.
...
- The left-bar Triforce anchor uses the shared exclusive root switch for click-open.
...
**Current path**: `main.lua` `popup_toggle_action()` + `modules/ui_builder.lua` + `bin/popup_switch`
- Menu anchor click scripts use the compiled `popup_switch switch <item>` action,
  or the matching `plugins/popup_manager.sh` mode in portable setups. The helper
  sends one SketchyBar request that closes other roots and every child before
  toggling the target last.
- Child rows use `popup_switch submenu <item>`: other children close, the
  requested child toggles, and its complete registered ancestor chain stays
  open. This applies to Music, Front App, Control Center, and nested rendered
  Apple/menu children within the bounded manifest (128 roots, 128 children, 512
  ancestor relations, and 127 bytes per item name).
- Generated clicks carry the current manifest generation token. A failed
  publish, stale manifest, invalid delimiter, or over-limit registry therefore
  falls back to toggling only the requested target rather than closing names
  from an older configuration.
- The generated Control Center skhd shortcut uses
  `scripts/invoke_popup_click.sh` to forward through the click currently
  installed on the live item. This keeps the external shortcut on the same
  generation without persisting a token that changes on every reload.
- The dedicated `popup_switch` binary name is intentional. A work Mac that
  pulls new Lua/shell sources but still has an old event-only `popup_manager`
...
- Do not route `front_app`, `control_center`, Triforce, Music, or right-side
  ...
  status/hover/detail work; the visibility switch completes before asynchronous
  refresh generated by `popup_toggle_action(<item>)` or
  `ui.toggle_then_refresh_async(...)`.
- Do not replace the bounded switch with query/focus wrappers unless physical
  clicks have been validated. Script-only tests can pass even when the mouse
  event never reaches the item.
...
- Click switching reads one atomically published
  `${TMPDIR}/sketchybar_popup_topology` manifest. If it is absent or malformed,
  the helper sends only the requested toggle instead of retired fallback names.
...
```
Expected: click scripts contain `popup_switch switch "<item>"` (or the portable
`popup_manager.sh switch` equivalent), keep `popup.drawing=toggle` only as the
missing-helper fallback, and do not route through `BARISTA_*_ACTION=click ...`.
