...
- **Native System Info Detail Refresh:** routine system-info events keep the `system_info_widget` compact anchor path, while clicks toggle the popup immediately and start `system_info_popup_helper popup_refresh` in the background. The popup alias is built from the same source under a new name so an older routine-only binary cannot be mistaken for the new entrypoint. `modules/items_right.lua` sends the exact enabled subset of `cpu,mem,disk,net,swap,uptime,procs`; the helper gathers only those rows and applies them as one bounded SketchyBar Mach update. Its external probes use bounded absolute paths for Data-volume disk usage, the default-route/Wi-Fi network, and the top process. Missing helpers and native invocation, payload, or IPC failures use the strict `plugins/system_info.sh popup_refresh` fallback, while individual probe failures render safe placeholders. Lua-only mode explicitly ignores a stale routine binary. In a 20-pair same-daemon live profile, native detail refresh measured 36.418 ms median / 37.010 ms p95 versus 127.795 / 129.427 ms for the shell fallback (3.51x by medians).
