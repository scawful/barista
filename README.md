...
- **Progressive Popup Disclosure:** on the fully populated Personal/Yabai models, Music's initial surface drops from 24 rows to 13 by moving secondary launchers into `More Apps` and `Kits + Folders` children, Front App's enabled root drops from 18 rows to 10 while its `More Actions` child stays at 17 rows, and Control Center drops from 23 rows to 12 by moving its 11 Layout Ops/App Defaults rows into `More Layout Controls`. A populated Apple menu likewise replaces LM Studio, ChatGPT, Claude, and Cursor with one click-only `AI Apps` row, reducing this Mac's root from 15 rows to 12 while keeping AFS Browser, Cortex, and Ghostty direct; zero or one rendered AI launcher stays direct instead of creating a child. Front App keeps Quit, Float, Adopt Space Mode, and Fullscreen direct while redundant root chrome is omitted. These children open only on click; the shared popup switch closes sibling children and any children belonging to the previous root, while child actions close every containing popup level. Disabled/no-Yabai Front App keeps app actions direct and omits its child; Control Center likewise omits its child when controls are disabled or unavailable. Same-daemon A/Bs cut Front App direct-layout median by 23.9% for 18→13 rows and another 11.2% for 13→10; Apple's 15→12 first-paint renderer median improved by 8.155 ms (10.2%). See the performance audit for full results.
