...
- **Adaptive Runtime Context:** `scripts/runtime_context.sh` prefers the compiled `bin/runtime_context_helper` for front-app/focused-space work, while its portable media path uses one bounded player snapshot, probes less often when idle, and publishes media/output caches only when bytes change. Explicit Lua-only profiles keep compiled helpers disabled even if an old binary is present.
