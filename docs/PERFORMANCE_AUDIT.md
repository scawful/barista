...
*   **Update:** shared click-only child popups keep the frequently used root rows shallow. The fully populated Music root moves from 24 rows to 13 through `More Apps` and `Kits + Folders`; the Yabai-enabled Front App root now moves from 18 rows to 13 while the existing `front_app.more` child is renamed `More Actions` and grows from 12 rows to 17; and the fully enabled Control Center root moves from 23 rows to 12 through `More Layout Controls`, whose 11 rows contain Layout Ops and App Defaults. Front App keeps Quit, Float, Adopt Space Mode, and Fullscreen on the root; Hide, Force Quit, Sticky, Topmost, and Center join the existing presets and move actions in the child.
