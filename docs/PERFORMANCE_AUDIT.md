...
*   **Update:**
    ...
    - `plugins/calendar.sh` applies the exact 14 calendar/task rows in one batched SketchyBar invocation
    - the popup no longer materializes the derived weekend countdown, month-progress line, or week/day-of-year footer; the month header, six-week grid, date/time summary, optional meeting, and four task rows remain unchanged
...
*   **Result:** closed task/calendar surfaces add no steady-state task parsing or popup-update work, while configured users still get immediate refresh after interaction or task mutation. In a 60-pair frozen-source replay with one captured read-only syshelp payload, the bundled Task Pulse refresh median fell from `103.869 ms` to `50.746 ms` (`51.14%`) and p95 from `106.633 ms` to `51.969 ms`. The live configured-syshelp lane fell from `125.943 ms` to `74.151 ms` (`41.12%`) and p95 from `130.465 ms` to `76.906 ms`. Python starts dropped from three to one on the bundled path; explicit executable focus-provider overrides retain compatibility with two starts. Normalized snapshots and exact SketchyBar arguments matched in every lane and sample. Artifacts: `/tmp/barista_task_pulse_ab_20260727/artifacts/replay-release-54f4909.json` (SHA-256 `a3a2417c267ea61ea7fa340a8edfdd659d05d2f413055f38c62b2ec08916cc6c`), `/tmp/barista_task_pulse_ab_20260727/artifacts/live-release-54f4909.json` (SHA-256 `4484da316629844b5aef294d63cf3f2da39cffdadbd380976c044ac2c3567907`), and `/tmp/barista_task_pulse_ab_20260727/artifacts/process-count-release-54f4909.json` (SHA-256 `014c7f3cdfeccd274d0f6b492bcb97bc13549eb7183bb6f3d4f617950f693090`). A later strict same-daemon first-paint A/B ran four warmups plus 50 balanced pairs against the exact live 17-configured/16-drawing Clock popup and a 13-drawing-row proxy hiding only the three retired metadata rows. Process median/p95 improved from `49.727 / 57.234 ms` to `41.949 / 51.307 ms`; renderer median/p95 improved from `57.740 / 65.940 ms` to `48.831 / 58.069 ms` (15.43% / 11.94%). The compact model won 43/50 renderer pairs with a `7.330 ms` paired median benefit and bootstrap 95% CI `[5.695, 9.275]`; 7,767 direct queries required zero retries. Every preregistered gate cleared, supporting the final true 17→14 configured-item reduction. Artifact: `/tmp/barista_clock_metadata_ab_20260805/clock-metadata-proxy-ab-0878496-20260805.json` (SHA-256 `f8dd238c5e05a9903b20d79c72b36f1f301dd6e70e00dcd4b70f8268465fbebb`; harness SHA-256 `c6e9d254fd95bb757b9dd035dd239b086e9897093dcc0ae1c4b83d9df3ebe099`).
...
*   **Update:**
    ...
    - the configured popup is now 12 functional rows instead of 14: the duplicate `Audio` header and decorative separator are not materialized, while all state, route, transport, mute, and settings rows keep their existing ownership and order
    - neither removed row was a native-helper target; the portable shell fallback no longer emits the former `volume.header` mutation, so compiled and Lua-only/work-machine layouts share the same compact topology without changing the helper's ten-update contract (anchor plus nine mutable popup targets)
...
*   **Result:** the original 20-pair alternating live sample reduced median detail-refresh latency from 281.08 ms to 49.32 ms (82%, 5.70x) and p95 from 298.94 ms to 55.61 ms (81%, 5.38x). On the current hardware-controlled M4 output, 20 randomized before/after pairs changed helper exit `3` plus shell fallback into native exit `0`, reducing median refresh latency from 429.62 ms to 103.83 ms (75.8%, 4.14x) and p95 from 1131.56 ms to 247.49 ms (78.1%). System load averaged above 70 during the latter run, so exit behavior and direction are stronger evidence than the absolute latency. A later same-daemon 50-pair first-paint A/B isolated the common 10→8 drawing-row surface: renderer median improved from 48.889 ms to 44.047 ms (9.90%), with a 3.042 ms paired median benefit (bootstrap 95% CI `[1.538, 5.325]`) and 37/50 compact wins, but renderer p95 regressed from 54.417 ms to 56.031 ms (2.96%). The preregistered evidence gate therefore did **not** clear, and no tail-latency improvement is claimed; the two-row removal ships as a compact UX and true configured-item reduction. Artifact: `/tmp/barista_volume_redundant_rows_ab_20260804/volume-redundant-rows-ab-e97dd96-20260804.json` (SHA-256 `1845395008fdfaa935f4aecb3acaa7b3d16ac349ee40b515396f73d3c1340bb9`). No widget or polling timer was added.
...
*   **Result:** on the enabled-action topology, a same-session sequential 60-run renderer-only sample reduced median apply latency from 27.173 ms to 3.563 ms (86.9%) and p95 from 29.133 ms to 3.773 ms (87.0%). A same-session sequential 25-run live popup-refresh sample reduced end-to-end median from 79.948 ms to 54.832 ms (31.4%) and p95 from 86.069 ms to 58.181 ms (32.4%). Both observed profiles ran the old path first against the same settled Ghostty/Tiled context. At that stage the fresh-context wrapper/discovery path remained the dominant portion; section 0j records the later direct-helper dispatch cleanup without adding a renderer.
...
### 0j. Direct Native Front-App Popup Refresh (Verified)
*   **Files:** `plugins/front_app.sh`, `tests/test_front_app_plugin.sh`
*   **Update:**
...
*   **Files:** `helpers/popup_manager.c`, `plugins/popup_manager.sh`, `modules/ui_builder.lua`, `modules/submenu_registry.lua`, `main.lua`, `modules/items_left.lua`, `modules/items_right.lua`, `tests/test_popup_manager.sh`, `tests/test_popup_manager_dispatch.c`, `tests/test_binary_resolver.lua`
*   **Update:**
    ...
    - default macOS compiled clicks now serialize the same target-last vector directly to the canonical SketchyBar Mach service, bounded to 1,027 tokens / 64 KiB; custom binaries, non-Apple builds, oversized payloads, explicit `BARISTA_POPUP_MACH_DISABLE=1`, and failures before an accepted send retain the exact argv path
    - an accepted send is at-most-once because its final operation toggles the target: reply timeout or malformed response is treated as delivered, an explicit `[!]` response reports failure, and neither condition retries or replays through the CLI
    - deterministic dispatch tests cover direct delivery, unconfirmed/error no-replay, exact full-vector fallback, custom/canonical binary boundaries, the 128-byte `BAR_NAME` boundary, stale-generation target-only behavior, and the maximum 1,027-token / 45,596-byte accepted topology; Lua-only resolution never probes a stale native helper
...
*   **Direct transport result:** a final-source, same-daemon randomized A/B used five warmup pairs and 30 measured pairs with all 11 roots / 4 children forced off around each lane. Process completion fell from `56.573 ms` median / `62.662 ms` p95 on exact CLI execution to `51.400 / 62.256 ms` on direct Mach dispatch, a `4.168 ms` paired median saving (19/30 direct wins). Renderer-observed clock open fell from `63.309 / 70.845 ms` to `59.126 / 69.324 ms`, a `5.354 ms` paired median saving (20/30 direct wins). The 743-byte payload remained byte-identical to the pre-change oracle (SHA-256 `8eefeb7fc78f04b213868c1112615246fc869bd727a383f00c679a7533d3976b`); SketchyBar, Lua, and skhd PIDs, logs, state, topology, bar query, hover state, and final all-off topology stayed unchanged. Artifact: `/tmp/barista_popup_mach_candidate_20260731/live-direct-vs-cli-clock-final-2ae983c-20260731.json` (SHA-256 `d5731219635aa82b9ae9fc5332736127f02dac06dc3768329dc89e9e70dfa20f`).
...
*   **Files:** `modules/items_right.lua`, `helpers/system_info_widget.c`, `helpers/CMakeLists.txt`, `plugins/system_info.sh`, `tests/test_items.lua`, `tests/test_system_info_memory.sh`, `tests/test_system_info_widget.c`
*   **Update:**
    - the default compiled runtime keeps periodic compact-anchor ownership in `widget_manager daemon`; daemon-disabled and portable paths retain the event wrapper plus routine `system_info_widget`, and neither routine path targets popup rows
    - the click-only `system_info_popup_helper` alias is built from the same source under a new name so a pre-upgrade routine binary cannot accidentally receive the new action
    ...
    - direct APIs cover CPU, active+wired+compressed Mach memory, SystemConfiguration Wi-Fi-interface enumeration, interface addresses, swap, and uptime; remaining data comes from bounded absolute-path probes: `/bin/df` against `/System/Volumes/Data` when present, `/sbin/route` for the default route, optional `/usr/sbin/networksetup -getairportnetwork` for the SSID, and `/bin/ps -Ar -o pcpu=,pid=,ucomm=` for the system-wide top process
    - the top-process probe keeps `ps` because its Apple entitlement can see protected processes that pure `libproc` cannot; asking it for PID plus the dependable accounting name avoids formatting every executable path, then `proc_pidpath`/`proc_name` restore the full basename when permitted
    - actionable Top CPU now replaces the duplicate standalone Activity Monitor row while enabled; disabling Top CPU restores the static action, while `system_info_items.actions=false` preserves the metric allowlist but makes Top CPU informational and omits both static launchers
...
*   **Result:** the first native-detail wave measured 36.418 ms median / 37.010 ms p95 versus 127.795 / 129.427 ms for the shell fallback in 20 randomized same-daemon pairs (3.509x by medians). Phase attribution then measured the retired Wi-Fi hardware-list probe at 14.998 ms median and the full-path top-process probe at 22.878 ms. In the release profile of the final binary (five warmups, 50 randomized samples per helper case), network-only work measured 24.781 / 25.673 ms median/p95, process-only 16.767 / 17.136 ms, and the full helper 25.376 / 25.978 ms. The full helper fell 11.284 ms / 30.8% by median and 18.410 ms / 41.5% by p95 versus the phase baseline; 20 exact live updates measured 25.463 / 26.533 ms with stable PID and zero log growth. A later controlled 30-sample live profile corrected the earlier confounded launcher attribution: `/bin/zsh -f -c ':'` measured 5.332 / 5.809 ms median/p95, exact background native-refresh dispatch 5.272 / 11.626 ms, native helper completion 40.312 / 46.872 ms, direct popup open 79.627 / 85.288 ms, full configured launch 79.883 / 87.088 ms, and refreshed-label observation 110.029 / 137.852 ms because open-popup polling queries add latency. Artifact: `/tmp/barista_system_info_click_profile_controlled_20260722.json` (SHA-256 `5489f79fd138ba98cdd8c4f1102a65e42213e8c24896dc7b31623effde331989`). A targeted eight-to-seven-row dedupe run measured a 2.851 ms median reduction (artifact `/tmp/barista_system_info_dedup7_ab_20260722.json`, SHA-256 `7eccad5b13ccaef3fa327e14c932f35f6b964bd21f96c7c8a1e0f41352402e41`), but a stricter randomized progressive-root A/B found five rows at 78.000 / 87.028 ms versus synthetic seven rows at 79.157 / 85.973 ms, a noisy -0.446 ms paired median with each model winning 15/30 samples. The real three-row child itself took 39.314 / 81.853 ms to open, so the nested prototype was rejected and no root-speed claim is made. Artifact: `/tmp/barista_system_info_progressive_root_ab_20260722.json` (SHA-256 `91fc021b7fd86b16fa89d2dca0d41d63a9729daae93cd11f505dbe0f0f738bc1`). The remaining bottleneck is SketchyBar/WindowServer nonempty-popup materialization, not shell/env dispatch or metric collection. Memory GiB matched the daemon anchor, the network label stayed unchanged, and the full `TR-1000 App` name survived PID hydration. The release and progressive profiles explicitly recorded stable daemons with zero log growth; the controlled click and row experiments restored the popup/topology after sampling. Release artifact: `/tmp/barista_system_info_native_discovery_release_20260722.json` (SHA-256 `7181250ab4ea96475f6a16c4de94e0e33eb304bf856c5ed041933064c4a454e6`).
...
*   **Update:**
    ...
    - focused refreshes persist one atomic, versioned `space + display` context after successful apply; same-display moves demote the previous space to idle, cross-display moves retain it as visible-inactive, and ambiguous or failed state falls back to the full visual path before authoritative success
    - previous and current focused styles now share one SketchyBar request; `front_app_switched` waits outside the visual lock for active topology to finish, bypasses only the authoritative cooldown while retaining front-app debounce when it acquires the visual lock directly, applies the latest event afterward, and fails open after a two-second stale-lock bound; ordinary visual-lock contention uses the framework-free `bin/file_lock` helper to apply `flock(2)` to inherited shell descriptors on macOS 13+ (lazily compiling it from a normal source checkout when needed), the Python standard library on the same inherited descriptor in Lua-only/restricted-work mode, or the standard `flock` CLI on Linux, fails closed if no safe lane-specific backend is available, and keeps one waiter for up to six seconds, which bypasses stale markers, re-reads live focus context, and opens the retry slot before sampling so a post-snapshot event can queue next while additional contenders coalesce
    - startup removes the prior runtime's selected context with its authoritative marker, forcing any early active event through a complete full visual pass before the new runtime can publish success
...
*   **Update:**
    ...
    - disabled phase probes now assign `0` directly in the parent shell; cached style properties, normal `space.*` state paths, and their root also stay in that shell instead of being reconstructed through nested command substitutions
*   **Result:** detailed attribution remains available when explicitly enabled. On the normal nine-space/two-visible full path, the parent-shell changes remove 19 disabled-phase and 42 style-path subshells. In a 100-pair isolated release A/B using the deployed native lock boundary against the correctness-only implementation, median fell from `72.868 ms` to `49.180 ms` and p95 from `77.460 ms` to `52.655 ms`; the optimized path won 100/100 pairs, with a `23.984 ms` median paired saving, while SketchyBar arguments, selected context, and style-state hashes matched in every pair. Artifact: `/tmp/barista_space_visual_phase_style_release_ab_20260726.json` (SHA-256 `9777c103c5bf76bf98da7c663cec1e7f86270f7c85e6ce011829fcc496f2ae07`; optimized source SHA-256 `448eab9efdfd853dbd65e86bc0a0026f0feb4abf59c0f0caa67e83b63fca77b5`; one-minute load median `3.188`, max `3.239`).

### 3d. Post-Config Startup Ordering (Verified)
*   **Files:** `main.lua`, `modules/items_left.lua`, `modules/menu_renderer.lua`, `modules/runtime_startup.lua`, `plugins/reload_sketchybar.sh`, `tests/test_reload_sketchybar.sh`
*   **Update:**
    ...
    - core-item readiness now covers a ten-second startup tail and includes a final boundary query; a core item that appears at the old five-second edge no longer causes a second raw reload, while a genuinely missing item retains that recovery path
    - reload locks carry the owner PID and refuse stale-lock reclamation while that process is alive, so the longer bounded readiness path cannot reopen the overlapping-restart race
...
*   **Result:** item registration commits before external startup effects run, the normal delayed path no longer creates shell sleeper processes that can survive a process-replacing reload, and missing optional widgets cannot leave phantom popup-manager targets. A supported live restart on 2026-07-20 dispatched 111 initial plus two late post-config actions, sampled no `sleep 0.2`, `sleep 0.8`, or `sleep 1.0` processes, produced no new stdout/stderr item errors, and restored all queried core items plus both runtime sidecars; that run recorded 297 ms config wall time and 933 ms total reload time. After the inclusive readiness and owner-lock fix, a 2026-07-22 supported restart returned successfully in 4,975 ms, recorded one 1,406 ms internal `reload_time` event, and produced no second event during the next seven seconds; `front_app`, `system_info`, and `space.1` were queryable, the new SketchyBar PID stayed stable, and the System Info popup remained closed. This proves the redundant fallback config was avoided; `reload_time` is still the internal pre-sidecar metric rather than click-to-ready wall time.

### 3e. Progressive Popup Disclosure (Verified)
*   **Files:** `main.lua`, `modules/ui_builder.lua`, `modules/items_left.lua`, `modules/apple_menu_enhanced.lua`, `modules/integrations/music.lua`, `modules/integrations/control_center.lua`, `plugins/front_app.sh`
*   **Update:** shared click-only child popups keep the frequently used root rows shallow. The fully populated Music root moves from 24 rows to 13 through `More Apps` and `Kits + Folders`; the Yabai-enabled Front App first moved from 18 rows to 13 through its 17-row `More Actions` child and now renders a 10-row root by omitting its duplicate App header and two decorative separators; and the fully enabled Control Center moves from 23 rows to 12 through `More Layout Controls`, whose 11 rows contain Layout Ops and App Defaults. A populated Apple Apps section replaces its four rendered LM Studio, ChatGPT, Claude, and Cursor rows with one `AI Apps` child entry, moving the live root from 15 rows to 12 while AFS Browser, Cortex, and Ghostty remain direct. The child appears only when at least two eligible launchers render, so a sparse machine keeps its lone launcher direct. Front App keeps its dynamic title, state, location, Quit, Float, Adopt Space Mode, and Fullscreen rows direct while every secondary action stays in the unchanged child.
    Root toggles close their children before changing root state, and nested Apple actions now close every containing fly-out plus `apple_menu` in one SketchyBar request after the original action is dispatched. `items_left.lua` reports its child popup names and `main.lua` merges them with menu submenu metadata before writing the runtime registry. Disabled/no-Yabai Front App models keep the app actions on the root and omit the child; disabled and no-Yabai Control Center models also omit their child.
*   **Result:** every launcher/window action remains reachable while the first popup surface is shorter. The earlier 29-to-18 Front App disclosure pass was measured alongside Music: in 20 randomized live samples per root/path, the layout-only Music median fell from 92.43 ms to 66.54 ms (28.0%) with p95 108.56 ms to 79.48 ms; Front App fell from 107.54 ms to 77.30 ms (28.1%) with p95 115.09 ms to 94.72 ms. Configured-click medians also fell from 117.82 ms to 96.20 ms for Music and 127.59 ms to 115.10 ms for Front App. The Roland TR-1000 process remained near 100% CPU and after-state system load was higher; Front App's configured-click p95 was correspondingly noisy (136.64 ms to 156.32 ms), so the direct layout path is the primary acceptance signal.
    A separate same-daemon, 20-round randomized A/B reparented exactly the five new secondary rows between the 18-row and 13-row roots. Direct layout median fell from 88.26 ms to 67.16 ms (23.9%) and p95 from 112.22 ms to 108.91 ms (3.0%); the 13-row topology was faster in 15 of 18 complete direct pairs, with a -16.69 ms median paired delta. Aggregate configured-click medians were flat/noisy at 101.51 ms versus 102.52 ms, while p95 moved from 117.10 ms to 116.02 ms; paired configured samples favored the smaller root in 12 of 18 rounds but ranged from -43 ms to +41 ms. This variance indicates that the zsh/background-refresh launch masks the direct layout gain. Load stayed stable (5.59 to 5.16), the daemon PID stayed stable, logs added zero bytes, and four immediate JSON-null state queries were excluded.
    A fresh 2026-07-24 same-daemon, 20-round randomized A/B then moved only `front_app.sep0`, `front_app.app_header`, and `front_app.sep_presets` between the 13-row and 10-row roots. With the corrected absolute-yabai refresh path, direct layout median fell from 65.436 ms to 58.115 ms (11.2%) and p95 from 76.894 ms to 64.718 ms (15.8%); the 10-row model won 17/20 pairs with a -9.952 ms paired median. Configured-click median fell from 91.499 ms to 83.509 ms (8.7%) and p95 from 97.010 ms to 89.443 ms (7.8%); the compact root won 17/20 pairs with a -9.511 ms paired median. All 40 configured samples matched a fresh stable wrapper snapshot on their first verification attempt (`App · Logic Pro`, `Tiled · Float Space`, `Space 8 · Display 1`). PID `65521` and all three runtime sidecars stayed stable, both logs grew zero bytes/lines, `state.json` bytes/hash/mtime stayed unchanged, exact 13/17 ordering was restored, every popup finished closed, and no runaways remained. Artifact: `/tmp/barista_front_app_13_vs_10_ab_corrected_yabai_20260724.json` (SHA-256 `53e02910954815473c793076a8b2ee27350e16bd33b15c18c74057e6fcdcd326`).
    ...
    On 2026-08-04, a same-daemon Apple first-paint A/B ran four warmups plus 50 balanced paired rounds against the exact 15-row root and a 12-drawing-row stand-in that preserved AFS Browser, Cortex, and Ghostty direct. Process-completion median fell from `71.944 ms` to `64.563 ms` (10.3%) and p95 from `76.483 ms` to `70.086 ms`; renderer-observed median fell from `79.951 ms` to `71.796 ms` (10.2%) and p95 from `86.142 ms` to `82.978 ms`. The compact surface won 39/50 renderer pairs with an `8.184 ms` paired median benefit and bootstrap 95% CI `[5.886, 9.891]`; 7,056 row/state queries required zero retries. PIDs, exact baseline rows, state/topology hashes, logs, and the all-off popup state were restored. Artifact: `/tmp/barista_apple_progressive_ab_20260804/apple-progressive-disclosure-ab-b2150cb-20260804.json` (SHA-256 `9b73bbcce2002b69db3bd1b19ee71e1b5e67dc79926f00083ba8561956e5d949`; harness SHA-256 `d275c3ea30307d53525774030d526b56acacda704dada907a6cf24a1076178cc`). This isolates root first paint; functional child switching and dismissal are covered separately by tests and live acceptance.
...
6.  **Popup materialization cost:** default compiled popup switching now removes the bounded CLI-launch overhead; its final same-daemon A/B saved `4.168 ms` at process completion and `5.354 ms` at renderer-observed open. Initial latency is still dominated by SketchyBar/WindowServer layout and scales with visible row count: the Apple 15→12 first-paint A/B saved another `8.155 ms` at the renderer median. The child rows remain eagerly configured, so Apple is root-layout progressive disclosure rather than global item-count reduction; Volume and Clock instead remove redundant configured items outright. Volume's controlled 10→8 drawing-row A/B improved renderer median by 9.90% but regressed renderer p95 by 2.96%, so it does not establish a tail-latency win. Clock's strict 16→13 drawing-row A/B cleared the unchanged gate with 15.43% median and 11.94% p95 improvements. Keep root menus shallow instead of adding more hot-path transport layers.
