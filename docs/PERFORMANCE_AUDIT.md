...
6.  **Popup materialization cost:** default compiled popup switching now removes the bounded CLI-launch overhead; its final same-daemon A/B saved `4.168 ms` at process completion and `5.354 ms` at renderer-observed open. Initial latency is still dominated by SketchyBar/WindowServer layout and scales with visible row count: the Apple 15→12 first-paint A/B saved another `8.155 ms` at the renderer median. The child rows remain eagerly configured, so this is root-layout progressive disclosure rather than global item-count reduction; keep root menus shallow instead of adding more hot-path transport layers.
